...
export const selectIsAuthenticated = createSelector(selectFeature, (state: AuthState) => {
  if (!state.data) {
    return false;
  }
...
export const selectLoading = createSelector(selectFeature, (state: AuthState) => {
  return state.loading;
});

export const selectStatus = createSelector(selectError, selectLoading, (error, loading) => {
  return {
    error,
    loading
  };
